...
| Exchange support | S1 boundary, S4 build | Registered exchange modes are promoted only through fixture/testnet/sandbox-backed profiles, and broader exchange ids stay `generic-unverified`. See [exchange-support-matrix.md](exchange-support-matrix.md). | `safety`, `operator-usability`, `oss-polish` |
